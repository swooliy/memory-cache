...
class MemoryCache implements CacheInterface
{
     ...
     */
    public function __construct(array $columns, $size = 1024)
    {
        $this->table = new Table($size);

        foreach ($columns as $key => $value) {
            $this->table->column($key, $value['type'], $value['size']);
        }
    ...
    public function getMultiple($keys, $default = null)
    {
        ...
        foreach ($keys as $k) {
            if ($v = $this->table->get($k)) {
                $result[$k] = $this->table->get($k);
            }
        }
